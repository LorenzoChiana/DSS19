...
#------------------------------------------------------------------------------
diffdata = np.cumsum(diffdata)
trainPredict = np.cumsum(trainPredict)
testForecast = np.cumsum(testForecast)

plt.rcParams["figure.figsize"] = (10,8) # redefines figure size
plt.plot(diffdata) #blue
plt.plot(np.concatenate((np.full(1,np.nan),trainPredict))) #orange
plt.plot(np.concatenate((np.full(len(train)+1,np.nan), testForecast))) #green
plt.show()